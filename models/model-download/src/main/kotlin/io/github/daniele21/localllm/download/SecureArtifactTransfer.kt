...
internal class SecureArtifactTransfer(private val transport: DownloadTransport, policy: SecureDownloadPolicy) {
    ...
    private fun redirectState(state: TransferState.Continue, location: String?): TransferState {
    ...
    private fun completeFailure(code: DownloadErrorCode, detail: String): TransferState.Complete = TransferState.Complete(
        ArtifactTransferResult.Failure(DownloadError(code, detail)),
    )

    private fun cancelled(): ArtifactTransferResult.Failure = ArtifactTransferResult.Failure(
        DownloadError(DownloadErrorCode.CANCELLED, "Download cancelled"),
    )

    private sealed interface TransferState {
        data class Continue(val uri: URI, val redirectCount: Int) : TransferState
