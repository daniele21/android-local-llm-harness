...
| LLRT-9 | P2 | IN PROGRESS | Evaluation-only bounded batch orchestration is being established; native multi-sequence execution and throughput evidence remain separate gates. |
...
| LLRT-9A | IN PROGRESS | Add evaluation-only bounded batch planning/execution with exact ordered attribution and a serial compatibility path; do not wire production concurrency. |
...
LLRT-6 and LLRT-7 may therefore proceed experimentally with release defaults unchanged. LLRT-6 now waits on the curated KV-cache device matrix; LLRT-7 now waits on representative exact-artifact OpenCL device evidence. LLRT-9 may establish evaluation-only software orchestration without changing production decode policy. LLRT-8 follows representative LLRT-7 evidence; LLRT-10 still waits for reviewed measured CPU/memory/hardware profiles.
