...
- [`local-inference-activity-audit.md`](local-inference-activity-audit.md) — coordinates durable local inference attribution/history: privacy-safe technical telemetry persistence, separate encrypted prompt/output audit storage, verified Consumer identity, Activity UI, restart reconciliation and RedactGuard two-APK proof.
...
- [`background-process-lifecycle-hardening.md`](background-process-lifecycle-hardening.md) — coordinates detached logical inference jobs, Binder reconnect/reconciliation, started/background host execution and process-recovery evidence without coupling UI/transport lifetime to runtime/model lifetime.
