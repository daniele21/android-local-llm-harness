...
Deterministic release-identity integration evidence now exercises Harnex and RedactGuard with distinct signing identities. Physical Google Play Internal Testing on 2026-09-07 additionally confirmed the real current distribution topology: the Play-delivered Harnex and RedactGuard builds both report the same Play App Signing SHA-256 digest. That physical run therefore proves Consumer-first install-order reachability, explicit authorization and connectivity for the current Play topology; it does not claim distinct-signer Play evidence. The authorization design must support both topologies without relying on co-signing.
...
Physical Play Internal testing remains required before a stable promotion claim because Play App Signing is the real distribution environment that exposed the original assumption gap. The 2026-09-07 physical run satisfies the install-order/authorization/connectivity check for the current Play-delivered topology and records its actual signing identities. It does not substitute for deterministic distinct-signer coverage or other exact-release-candidate physical gates.
