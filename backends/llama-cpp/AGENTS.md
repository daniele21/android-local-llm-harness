...
This guide applies to `backends/llama-cpp/**` and supplements the repository-wide [`AGENTS.md`](../../AGENTS.md). It covers the Kotlin `InferenceBackend` implementation, JNI entry points, C++ ownership, native build configuration, GGUF inspection adaptation and host-native tests.
...
Trace both sides of a backend/native call before editing it:

```bash
rg '<backend-or-jni-symbol>' core/backend-spi core/runtime-core backends/llama-cpp apps/device-test-runner
rg --files backends/llama-cpp/src/main backends/llama-cpp/src/test backends/llama-cpp/src/test-native
```

## Local invariants

- Implement backend-neutral lifecycle/prompt/context/generation contracts from `core/backend-spi`; do not redefine them locally.
- The backend receives a runtime-adapted `BackendModelSource`; it does not import `models:model-store` or decide installation, verification or selection policy.
...
- Keep one native streaming decode path for prompt preparation, tokenization, sampling, prefill, decode, stop handling, metrics and cleanup. Aggregate above the native boundary when needed.
...
- Put backend-neutral model-source/capability/prompt/context/generation/cancellation contract changes in `core/backend-spi` before adapting them here.
- Put public application/consumer request and lifecycle changes in `core/contracts`.
- Put runtime scheduling, model verification/adaptation, model-switch and lifecycle policy in `core/runtime-core`; this module implements backend operations and native resource ownership only.
...
./gradlew :core:backend-spi:testDebugUnitTest :core:backend-spi:lintDebug \
  :backends:llama-cpp:compileDebugKotlin \
...
- backend SPI adaptation, JNI entry points, registries, native ownership or source decomposition change;
