...
internal object HarnessDiagnosticsExport {
    ...
    fun build(context: Context): String {
        ...
        return buildString {
            ...
            appendLine("historyCount=${baselineHistory.size}")
            baselineHistory.forEachIndexed { index, baseline -> appendBaseline("history", index, baseline) }
            appendLine()

            appendLine("[structured-logs]")
            appendLine("count=${logs.logs.size}")
            appendLine("sourceError=${logs.sourceError?.safeValue() ?: "None"}")
    ...
    private fun StringBuilder.appendRun(index: Int, run: GenerationRunRecord) {
        ...
        append(" completed=${run.completedAtEpochMs?.let(Instant::ofEpochMilli) ?: "Unavailable"}")
        append(" queueMs=${run.queueMs ?: "Unavailable"}")
        append(" modelLoadMs=${run.modelLoadMs ?: "Unavailable"}")
        append(" promptPlanningMs=${run.promptPlanningMs ?: "Unavailable"}")
        append(" contextCreationMs=${run.contextCreationMs ?: "Unavailable"}")
        append(" ttftMs=${run.timeToFirstTokenMs ?: "Unavailable"}")
        append(" prefillMs=${run.prefillMs ?: "Unavailable"}")
        append(" decodeMs=${run.decodeMs ?: "Unavailable"}")
        append(" totalMs=${run.totalMs ?: "Unavailable"}")
        append(" inputTokens=${run.inputTokens ?: "Unavailable"}")
        append(" outputTokens=${run.outputTokens ?: "Unavailable"}")
        append(" decodeTokPerSec=${run.decodeTokensPerSecond?.formatDecimal() ?: "Unavailable"}")
        append(" contextSize=${run.contextSize ?: "Unavailable"}")
        append(" promptTokens=${run.promptTokenCount ?: "Unavailable"}")
        append(" maxOutputTokens=${run.maxOutputTokens ?: "Unavailable"}")
        append(" preset=${run.presetId?.value?.safeToken() ?: "None"}")
        append(" presetVersion=${run.presetVersion ?: "Unavailable"}")
        append(" thinking=${run.thinkingMode?.name ?: "Unavailable"}")
        append(" temperature=${run.temperature ?: "Unavailable"}")
        append(" topP=${run.topP ?: "Unavailable"}")
        append(" topK=${run.topK ?: "Unavailable"}")
        append(" minP=${run.minP ?: "Unavailable"}")
        append(" presencePenalty=${run.presencePenalty ?: "Unavailable"}")
        append(" repeatPenalty=${run.repeatPenalty ?: "Unavailable"}")
        append(" repeatLastN=${run.repeatLastN ?: "Unavailable"}")
    ...
    private fun StringBuilder.appendResource(index: Int, resource: ResourceSnapshot) {
        appendLine(
            "resource[$index] time=${Instant.ofEpochMilli(resource.timestampEpochMs)} " +
                "pssBytes=${resource.processPssBytes ?: "Unavailable"} " +
                "nativeHeapBytes=${resource.nativeHeapBytes ?: "Unavailable"} " +
                "javaHeapBytes=${resource.javaHeapUsedBytes ?: "Unavailable"} " +
                "availableMemoryBytes=${resource.availableMemoryBytes ?: "Unavailable"} " +
                "lowMemory=${resource.lowMemory ?: "Unavailable"} thermal=${resource.thermalStatus.name}",
        )
    }
    ...
    private fun StringBuilder.appendBaseline(prefix: String, index: Int, baseline: BenchmarkBaseline) {
        appendLine(
            "$prefix[$index] app=${baseline.key.applicationId.value.safeToken()} " +
                ...
                "medianTtftMs=${baseline.medianTimeToFirstTokenMs?.formatDecimal() ?: "Unavailable"} " +
                "p95TtftMs=${baseline.p95TimeToFirstTokenMs?.formatDecimal() ?: "Unavailable"} " +
                "medianTotalMs=${baseline.medianTotalMs?.formatDecimal() ?: "Unavailable"} " +
                "p95TotalMs=${baseline.p95TotalMs?.formatDecimal() ?: "Unavailable"} " +
                "medianDecodeTokPerSec=${baseline.medianDecodeTokensPerSecond?.formatDecimal() ?: "Unavailable"}",
        )
    }
    ...
    private fun Double.formatDecimal(): String = "%.3f".format(Locale.ROOT, this)
