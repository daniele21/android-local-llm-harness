...
- Shared-runtime release/debug inference services deliberately omit a custom bind permission so a Consumer installed before Harnex is not permanently denied by Android permission grant timing. Reachability is not trust: Binder authorization derives the calling UID, exact installed package, current signer, persisted Harnex authorization and enabled use case on every privileged entry point. Known independent consumers are source-observed as pending, signer replacement fails closed until explicit reauthorization, and caller package matching remains exact.
- Emulator-only fault/control authority remains variant-scoped and separate from public inference. The ordinary emulator fault receiver stays signature-protected; independently signed cross-app CI may use only the `emulatorE2e` Host-process allowlisted bridge protected by `android.permission.DUMP`, which must remain absent from production variants. Never reuse or broaden the public inference bind surface for test control.
