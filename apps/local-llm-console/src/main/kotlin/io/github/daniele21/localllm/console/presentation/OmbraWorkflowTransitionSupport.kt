...
internal object OmbraWorkflowTransitionSupport {
    ...
    fun matchesActive(
        ...
        operationKind: OmbraOperationKind,
    ): Boolean = state.activeOperation == OmbraActiveOperation(operationId, operationKind)

    fun retryTarget(kind: OmbraOperationKind): OmbraRetryTarget =
        when (kind) {
    ...
    fun cancelReturnStage(kind: OmbraOperationKind): OmbraWorkflowStage =
        when (kind) {
