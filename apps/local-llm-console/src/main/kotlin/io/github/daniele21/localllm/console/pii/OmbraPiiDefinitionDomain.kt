...
internal object PiiDefinitionFactory {
    fun validateCustomDraft(draft: PiiDefinitionDraft, existingDefinitions: Collection<PiiDefinition>): PiiDefinitionValidation {
    ...
    fun createCustom(draft: PiiDefinitionDraft, existingDefinitions: Collection<PiiDefinition>): PiiDefinitionCreationResult {
        ...
        val id = nextCustomId(existingDefinitions.map { it.id }.toSet())
...
internal class PiiDefinitionSet private constructor(definitions: List<PiiDefinition>) {
    ...
    companion object {
        fun create(definitions: Collection<PiiDefinition>): Result<PiiDefinitionSet> = runCatching {
            ...
            require(
                definitions.count { it.source == PiiDefinitionSource.CUSTOM } <= PiiDefinitionLimits.MAX_CUSTOM_DEFINITIONS,
...
private fun containsUnsupportedControl(value: String): Boolean = value.any { character -> Character.isISOControl(character) }
