...
internal object OmbraAnalysisFindingValidator {
    fun validate(
        ...
        return OmbraChunkFindingValidation(
            ...
            issueCounts = issueCounts.toSortedMap(compareBy(Enum<*>::ordinal)),
        )
    }
    ...
    private fun validateOne(
        ...
        val typeId = runCatching { PiiTypeId.parse(raw.typeId) }.getOrNull()
            ?: return CandidateValidation.Invalid(OmbraFindingValidationIssue.INVALID_TYPE_ID)
        if (typeId !in selectedTypes) return CandidateValidation.Invalid(OmbraFindingValidationIssue.UNSELECTED_TYPE)
        if (raw.segmentId !in submittedIds) {
            return CandidateValidation.Invalid(OmbraFindingValidationIssue.UNKNOWN_SUBMITTED_SEGMENT)
        }
        val slice = sourceIndex.resolve(raw.segmentId)
            ?: return CandidateValidation.Invalid(OmbraFindingValidationIssue.UNKNOWN_SUBMITTED_SEGMENT)
        val occurrences = exactOccurrences(slice, raw.surface)
        if (occurrences.isEmpty()) return CandidateValidation.Invalid(OmbraFindingValidationIssue.SOURCE_SURFACE_NOT_FOUND)
        return CandidateValidation.Valid(ValidatedFinding(typeId, raw.surface, occurrences))
    }
    ...
    private fun exactOccurrences(slice: OmbraAnalysisSourceSlice, surface: String): List<SourceOccurrence> {
        ...
        var searchFrom = 0
        while (searchFrom <= slice.text.length - surface.length) {
            val localStart = slice.text.indexOf(surface, startIndex = searchFrom)
...
internal data class OmbraFindingConflict(
    ...
    override fun toString(): String =
        "OmbraFindingConflict(leftTypeId=$leftTypeId, leftOccurrence=$leftOccurrence, " +
...
internal object OmbraAnalysisFindingMerger {
    fun merge(chunks: List<OmbraChunkFindingValidation>): OmbraMergedAnalysis {
        ...
        val findings =
            ...
            }.sortedWith(compareBy({ it.occurrences.first().segmentId.value }, { it.occurrences.first().range.startInclusive }, { it.typeId.value }))
    ...
    private fun findConflicts(findings: List<ValidatedFinding>): List<OmbraFindingConflict> {
        val typedOccurrences = findings.flatMap { finding -> finding.occurrences.map { finding.typeId to it } }
            .sortedWith(compareBy({ it.second.segmentId.value }, { it.second.range.startInclusive }, { it.second.range.endExclusive }, { it.first.value }))
        val conflicts = mutableListOf<OmbraFindingConflict>()
        typedOccurrences.forEachIndexed { leftIndex, left ->
            typedOccurrences.drop(leftIndex + 1).forEach { right ->
                if (right.second.segmentId != left.second.segmentId) return@forEach
                if (right.second.range.startInclusive >= left.second.range.endExclusive) return@forEach
                if (left.first != right.first && left.second.range.overlaps(right.second.range)) {
                    conflicts += OmbraFindingConflict(left.first, left.second, right.first, right.second)
                }
            }
        }
        return conflicts
    ...
    private val SOURCE_OCCURRENCE_ORDER = compareBy<SourceOccurrence>({ it.segmentId.value }, { it.range.startInclusive }, { it.range.endExclusive })
}
