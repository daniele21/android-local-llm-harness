 ...
 */
internal class OmbraAnalysisChunkPlanner(
    private val policy: OmbraAnalysisPlanningPolicy = OmbraAnalysisPlanningPolicy(),
) {
    fun plan(
        segments: List<DocumentSegment>,
        definitions: List<PiiDefinition>,
        limits: ConsumerLimits,
    ): OmbraChunkPlanResult {
        ...
        while (pending.isNotEmpty()) {
            ...
            while (pending.isNotEmpty() && continueChunk) {
                ...
                val split = largestFittingPrefix(candidate, definitions, limits)
                    ?: return OmbraChunkPlanResult.Rejected(OmbraChunkPlanFailureCode.INPUT_OVERHEAD_EXCEEDS_LIMIT)
    ...
    private fun fits(
        definitions: List<PiiDefinition>,
        segments: List<OmbraAnalysisSegmentData>,
        limits: ConsumerLimits,
    ): Boolean {
        val payload = OmbraAnalysisDataSerializer.serialize(definitions, segments)
        return OmbraAnalysisProtocol.instruction.length +
            policy.templateOverheadCharacters +
            payload.length <= limits.maxInputCharacters
    }
