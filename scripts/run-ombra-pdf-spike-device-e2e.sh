...
TEST_CLASSES="io.github.daniele21.localllm.console.document.OmbraPdfSpikeInstrumentedTest,io.github.daniele21.localllm.console.document.OmbraPdfIsolationInstrumentedTest,io.github.daniele21.localllm.console.document.AndroidOmbraDocumentExtractorInstrumentedTest"
...
normalized export round-trip, glyph policy, process isolation and the production extractor adapter.
EOF
}
...
if [[ -z "$SDK_INT" || "$SDK_INT" -lt 26 ]]; then
    echo "Error: OMBRA PDF runtime suite requires Android API 26 or newer; device reports '$SDK_INT'." >&2
    exit 1
fi
...
echo "Running isolated PdfBox and production extractor acceptance evidence..."
./gradlew \
...
echo "OMBRA PDF runtime suite passed. Privacy-safe summary: $REPORT"
