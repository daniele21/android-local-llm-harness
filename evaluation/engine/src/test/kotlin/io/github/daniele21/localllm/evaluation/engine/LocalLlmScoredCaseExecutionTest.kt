...
class LocalLlmScoredCaseExecutionTest {
    @Test
    fun `scores answer output and correlates exact request telemetry`() = runBlocking {
        val client = FakeClient(completed())
        var correlatedRequest: RequestId? = null
        val telemetry = EvaluationTelemetryCorrelationPort { requestId ->
            correlatedRequest = requestId
            EvaluationCaseMetrics(timeToFirstTokenMs = 31L, totalMs = 120L, outputTokens = 12)
        }
        val execution = execution(client, telemetry)
        ...
        assertEquals(REQUEST_ID, correlatedRequest)
        assertEquals(31L, result.value.metrics.timeToFirstTokenMs)
        assertEquals(120L, result.value.metrics.totalMs)
        assertEquals(12, result.value.metrics.outputTokens)
    }

    @Test
    fun `telemetry failure does not fabricate metrics or discard quality result`() = runBlocking {
        val execution = execution(
            client = FakeClient(completed()),
            telemetry = EvaluationTelemetryCorrelationPort { error("telemetry unavailable") },
        )

        val result = execution.execute(CONFIG, CASE_ID, BINDING, SESSION_ID) as EvaluationStepResult.Success

        assertEquals(EvaluatorOutcomeCode.CORRECT, result.value.outcome?.code)
        assertNull(result.value.metrics.timeToFirstTokenMs)
        assertNull(result.value.metrics.totalMs)
        assertNull(result.value.metrics.outputTokens)
    }
    ...
    fun `request identity mismatch fails before generation`() = runBlocking {
        ...
        val execution = LocalLlmScoredCaseExecution(
            ...
            requestFactory = EvaluationCaseGenerationRequestFactory { _, _, binding, _ ->
        ...
        assertNull(client.generatedRequest)
    }

    @Test
    fun `terminal runtime failure remains typed generation failure`() = runBlocking {
        val failed = GenerationEvent.Failed(
            requestId = REQUEST_ID,
            error = io.github.daniele21.localllm.contracts.LocalLlmError.NativeRuntime("fixture failure"),
        )
        val result = execution(FakeClient(failed)).execute(CONFIG, CASE_ID, BINDING, SESSION_ID) as EvaluationStepResult.Failure
    ...
    private fun execution(
        ...
        client = client,
        caseSource = EvaluationCaseDefinitionSource { _, caseId -> case().takeIf { it.id == caseId } },
        requestFactory = EvaluationCaseGenerationRequestFactory { _, _, binding, sessionId -> request(binding, sessionId) },
        telemetry = telemetry,
    )
    ...
    private fun completed() = GenerationEvent.Completed(
        requestId = REQUEST_ID,
