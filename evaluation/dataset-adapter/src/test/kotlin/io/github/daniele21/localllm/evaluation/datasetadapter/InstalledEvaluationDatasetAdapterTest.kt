...
class InstalledEvaluationDatasetAdapterTest {
    @Test
    fun `preflight and case source consume the exact installed canonical pack`() = withFixture { fixture ->
        ...
        assertEquals(listOf("reasoning", "structured"), adapter.categories(config)?.map { it.id.value })
    }

    @Test
    fun `preflight rejects digest mismatch and sample case outside installed pack`() = withFixture { fixture ->
        val installed = fixture.install()
        val adapter = InstalledEvaluationDatasetAdapter(fixture.root)
    ...
    @Test
    fun `missing installed identity fails closed`() = withFixture { fixture ->
        val adapter = InstalledEvaluationDatasetAdapter(fixture.root)
