...
internal fun registrationSuccess(token: HostClientToken, negotiatedMinor: Int, features: List<String>) =
...
internal fun prepareResult(operationId: String, result: PrepareResult) =
    PrepareResultParcel(
...
internal fun sessionSuccess(operationId: String, externalSessionId: String) =
    SessionResultParcel(operationId, externalSessionId, null)

internal fun sessionFailure(operationId: String, error: WireErrorParcel) = SessionResultParcel(operationId, null, error)

internal fun generationFailure(externalRequestId: String, error: WireErrorParcel) =
...
internal fun LedgerFailure.toHostWireError(): WireErrorParcel =
    when (this) {
        ...
        LedgerFailure.SESSION_LIMIT, LedgerFailure.REQUEST_LIMIT, LedgerFailure.CONNECTION_LIMIT -> wireError(WireErrorCodes.CLIENT_BACKPRESSURE)
        LedgerFailure.DUPLICATE_EXTERNAL_SESSION_ID, LedgerFailure.DUPLICATE_EXTERNAL_REQUEST_ID -> wireError(WireErrorCodes.INVALID_WIRE_REQUEST)
...
internal fun wireError(code: String): WireErrorParcel =
    when (code) {
