...
class WireProtocolException(val wireCode: String, override val message: String) : IllegalArgumentException(message)

data class NegotiatedProtocol(val minor: Int, val enabledFeatures: Set<String>)
...
internal fun validateIdentifier(value: String, maxCharacters: Int, label: String) {
...
internal fun validateBoundedContent(value: String, maxCharacters: Int, label: String) {
    requireWire(value.isNotBlank(), "$label must not be blank")
    requireWire('\u0000' !in value, "$label must not contain NUL")
...
internal fun requireWire(condition: Boolean, message: String, code: String = WireErrorCodes.INVALID_WIRE_REQUEST) {
    if (!condition) {
...
internal fun transportFailure(message: String): WireProtocolException = WireProtocolException(WireErrorCodes.TRANSPORT_FAILURE, message)
