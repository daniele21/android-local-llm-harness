...
class MemoryAdmissionController(private val budget: RuntimeMemoryBudget) {
    fun decide(
        ...
        if (observation.lowMemory == true) {
            return MemoryAdmissionDecision.Reject(MemoryAdmissionRejectReason.PLATFORM_LOW_MEMORY)
        }
        if (request.resource == MemoryAdmissionResource.CONTEXT &&
            request.residency.residentContexts >= budget.maxResidentContexts
        ) {
            return MemoryAdmissionDecision.Reject(MemoryAdmissionRejectReason.RESIDENT_CONTEXT_LIMIT)
        }
        ...
        if (budget.requireHeadroomObservation && !hasAvailableSignal && !hasProcessSignal) {
            return MemoryAdmissionDecision.Reject(MemoryAdmissionRejectReason.HEADROOM_OBSERVATION_REQUIRED)
        }

        availableMemoryDecision(observation, request)?.let { return it }
        processPssDecision(observation, request)?.let { return it }
        return MemoryAdmissionDecision.Allow
    }
